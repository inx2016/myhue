...
<script type="text/html" id="notebook-menu-buttons-${ suffix }">
  ...
  % if ENABLE_PRESENTATION.get() and not IS_EMBEDDED.get():
    <!-- ko with: selectedNotebook() -->
      <div class="btn-group">
        ...
        css: {'btn-inverse': $root.isPresentationMode()}, attr: {title: isPresentationMode() ? '${ _ko('Exit presentation') }' : '${ _ko('View as a presentatio') }'}">
  ...
  % if ENABLE_PRESENTATION.get() and not IS_EMBEDDED.get():
    <!-- ko with: selectedNotebook() -->
      <div class="btn-group">
        <a class="btn" data-bind="click: function() { isPresentationMode(!isPresentationMode()); },
        css: {'btn-inverse': $root.isPresentationMode()}, attr: {title: isPresentationMode() ? '${ _ko('Exit presentation') }' : '${ _ko('Export as a script') }'}">
          <i class="fa fa-line-chart"></i>
        </a>
        <button>test</button>
        <!-- ko if: $root.canSave() -->
